  ...
  return (
    <div className="p-4">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-4">
        ...
        <div>
          <label className="block">{productName ? `${productName} - Tên sản phẩm:` : 'Tên sản phẩm:'}</label>
          <input {...register('name', { required: true })} className="w-full border p-2" />
        </div>
        <div>
          <label className="block">Giá:</label>
          <input {...register('price', { required: true })} className="w-full border p-2" />
        </div>
        <div>
          <label className="block">Dung lượng:</label>
          <input {...register('storage', { required: true })} className="w-full border p-2" />
        </div>
