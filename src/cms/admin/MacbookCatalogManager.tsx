...
const MacbookCatalogManager: React.FC = () => {
  ...
  const [selectedMacbookCatalogId, setSelectedMacbookCatalogId] = useState<
    string | null
  >(null);
  ...
  const handleDeleteMacbookCatalog = async () => {
    if (selectedMacbookCatalogId) {
      try {
        ...
        const errorMessMacbookCatalog = isIErrorResponse(error)
          ? error.data?.message
  ...
  return (
    <div className="w-full pb-10 xl:pb-0">
      ...
      />
      <ModalCreateMacbookCatalogPageAdmin
