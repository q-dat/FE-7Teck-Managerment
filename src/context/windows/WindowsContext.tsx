...
interface WindowsContextType {
  ...
  error: string | null;
  getAllWindows: (filter?: { w_cat_status?: number; name?: string }) => void;
...
export const WindowsContext = createContext<WindowsContextType>(defaultContextValue);
...
export const WindowsProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Get All Windowss
  const getAllWindows = useCallback(async (filter?: { w_cat_status?: number; name?: string }) => {
    await fetchData(
  ...
  // Create Windows
  const createWindows = useCallback(async (windowsData: FormData): Promise<AxiosResponse<any>> => {
    return await fetchData(
      () => createWindowsApi(windowsData),
      data => {
        if (data?.windowsData) {
          setWindows(prevLaptopWindows => [...prevLaptopWindows, data?.windowsData]);
        }
      },
      'create'
    );
  ...
  // Update Windows
  const updateWindows = useCallback(async (_id: string, windowsData: FormData): Promise<AxiosResponse<any>> => {
    return await fetchData(
      () => updateWindowsApi(_id, windowsData),
      data => {
        if (data?.windowsData) {
          setWindows(prevLaptopWindows => prevLaptopWindows.map(w => (w._id === _id ? data?.windowsData : w)));
        }
      },
      'update'
    );
    ...
    async (_id: string) => {
      try {
        ...
        setWindows(prevWindows =>
          prevWindows.map(w => (w._id === _id ? { ...w, windows_view: (w.windows_view ?? 0) + 1 } : w))
        );
        ...
        if (response.data?.win) {
          setWindows(prevWindows => prevWindows.map(w => (w._id === _id ? response.data.win : w)));
        }
      } catch (error) {
        ...
        setWindows(prevWindows =>
          prevWindows.map(w => (w._id === _id ? { ...w, view: (w.windows_view ?? 0) - 1 } : w))
        );
      }
    },
    [windows]
  ...
  // Delete Windows
  const deleteWindows = useCallback(async (id: string): Promise<AxiosResponse<any>> => {
    return await fetchData(
      () => deleteWindowsApi(id),
      () => setWindows(prevLaptopWindows => prevLaptopWindows.filter(w => w._id !== id)),
      'delete'
    );
  }, []);
