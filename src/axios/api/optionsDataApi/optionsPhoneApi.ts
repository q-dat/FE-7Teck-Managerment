...
export const getAllOptionPhonesApi = () => {
  return axios.get<{ ottpionsPhones: IOptionPhoneData[] }>(
    "/api/options-phones",
  );
};
...
export const getOptionPhoneByIdApi = (_id: string) => {
  return axios.get<{ ottpionsPhone: IOptionPhoneData }>(
    `/api/options-phone/${_id}`,
  );
};
...
// Update OptionPhone
export const updateOptionPhoneApi = async (
  _id: string,
  ottpionsPhoneData: FormData,
) => {
