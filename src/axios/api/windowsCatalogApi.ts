...
export const getAllWindowsCatalogsApi = () => {
  return axios.get<{ windowsCatalogs: IWindowsCatalog[] }>('/api/windows-catalogs');
};

// Get WindowsCatalog By ID
export const getWindowsCatalogByIdApi = (_id: string) => {
  return axios.get<{ windowsCatalog: IWindowsCatalog }>(`/api/windows-catalog/${_id}`);
};
...
export const updateWindowsCatalogApi = async (_id: string, windowsCatalogData: FormData) => {
  const response = await axios.put(`/api/windows-catalog/${_id}`, windowsCatalogData, {
    headers: {
      'Content-Type': 'multipart/form-data'
    }
  });
