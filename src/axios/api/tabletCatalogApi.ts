...
export const getAllTabletCatalogsApi = () => {
  return axios.get<{ tabletCatalogs: ITabletCatalog[] }>(
    '/api/tablet-catalogs'
  );
};
...
export const getTabletCatalogByIdApi = (_id: string) => {
  return axios.get<{ tabletCatalog: ITabletCatalog }>(
    `/api/tablet-catalog/${_id}`
  );
};
...
export const updateTabletCatalogApi = async (
  _id: string,
