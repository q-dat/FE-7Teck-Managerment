...
const TabletDetailPage: React.FC = () => {
  const { id } = useParams();
  const { getTabletById, tablets } = useContext(TabletContext);
      ...
      getTabletById(id)
        .then(fetchedTablet => {
          if (fetchedTablet) {
            ...
            setSelectedImage(fetchedTablet.tablet_img);
          }
        })
        .catch(error =>
          console.error('Lỗi khi lấy dữ liệu máy tính bảng:', error)
        );
    }
  ...
  }, [tablets, tablet, id, getTabletById]);
  //
                    ...
                    <div key={group?.group}>
                      <details className="group transform divide-y-[1px] bg-primary bg-opacity-5">
                        ...
                        {group?.fields
                          .filter(
                            field =>
                              tablet?.tablet_catalog_id?.[group?.group]?.[
                                field?.field
                              ]
                          )
                          .map(field => {
