...
const TabletDetailPage: React.FC = () => {
  const { id } = useParams();
  const { getTabletById, tabletDetails } = useContext(TabletContext);
  ...
  useEffect(() => {
    scrollToTopSmoothly();
    fetchData();
    const cleanup = handleScrollButtons(scrollRef, Object.keys(tabletDetails).length, () =>
      updateScrollButtons(scrollRef, setIsLeftVisible, setIsRightVisible)
    );
    return cleanup;
  }, []);
                  ...
                  {tabletFieldMap.map(group => (
                    <div key={group?.group}>
                      <details className="group transform divide-y-[1px] bg-primary bg-opacity-5">
                        ...
                        {group?.fields
                          .filter(field => tablet?.tablet_catalog_id?.[group?.group]?.[field?.field])
                          .map(field => {
                            const fieldValue = tablet?.tablet_catalog_id?.[group?.group]?.[field?.field];
                            return (
                              <div
              ...
              {activeTab === 'details' && (
                <p
                  ...
                  dangerouslySetInnerHTML={{
                    __html: tablet?.tablet_catalog_id?.content
                  }}
                ></p>
              )}
            </div>
